...
export function useTransactionEditing(
  ...
  const startEdit = useCallback(
    (
      transaction: Transaction,
      field: 'details' | 'category' | 'property' = 'details'
    ) => {
      ...
      const categoryValue =
        transaction.category?.id === 'uncategorized'
          ? ''
  ...
  const saveEdit = useCallback(
    ...
    [editingId, editingCategory, editingProperty, properties, cancelEdit, toast]
  );
