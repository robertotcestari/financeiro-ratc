...
import type { CreateRuleRequest, UpdateRuleRequest, RuleWithRelations } from '@/lib/database/rule-management';
...
const ruleFormSchema = z.object({
  ...
  propertyId: z.string().optional().transform(val => val === 'none' ? undefined : val),
...
export default function RuleForm({ rule, formData, onSubmit, isSubmitting, onCancel }: RuleFormProps) {
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Prioridade da regra (0-100). Regras com maior prioridade são aplicadas primeiro.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Defina para qual categoria e propriedade as transações devem ser direcionadas.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <FormField
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                      ...
                      <FormControl>
                        ...
                        >
                          {field.value && field.value !== "none"
                            ? (() => {
                                const property = properties.find(p => p.id === field.value);
                                return property ? `${property.code}${property.description ? ` - ${property.description}` : ''}` : "Nenhuma propriedade";
                              })()
                            : "Nenhuma propriedade"}
                          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        </UIButton>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            ...
                            {properties.map((property) => (
                              <CommandItem
                                key={property.id}
                                value={`${property.code} ${property.description || ''}`}
                                onSelect={() => {
                  ...
                  <FormDescription>
                    Propriedade opcional a ser associada às transações que atendem esta regra.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configure os critérios que uma transação deve atender para ser categorizada por esta regra.
              Todos os critérios definidos devem ser atendidos.
            </CardDescription>
          </CardHeader>
