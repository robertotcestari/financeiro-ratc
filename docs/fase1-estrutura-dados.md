...
### 7. Revisão Manual e Transferências

Nesta fase, a categorização é realizada manualmente através da interface, com marcação `isReviewed`. A identificação de transferências permanece automática/assistida e não afeta o DRE.
...
```
Transaction → Identificar transferências → Categorizar manualmente:
  ├─ Se Transferência → Criar/Vincular Transfer → UnifiedTransaction (isTransfer=true)
  └─ Se Receita/Despesa → UnifiedTransaction (isTransfer=false, isReviewed opcional)
```
...
- [ ] Inserir imóveis
- [ ] (Futuro) Inserir regras de categorização

### 4. Criar Utilitários

- [ ] Funções de importação de transações
- [ ] Motor de identificação de transferências
- [ ] (Futuro) Motor de categorização automática
