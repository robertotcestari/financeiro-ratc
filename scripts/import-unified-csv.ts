  ...
  try {
    ...
    for (const record of records) {
      ...
      if (matchingTransaction) {
        ...
        notMatched++;
        if (notMatched <= 5) {
          console.log(`  ❌ Não encontrada: ${date.toLocaleDateString('pt-BR')} | ${accountName} | R$ ${amount.toFixed(2)} | ${description}`);
        }
      }
    }
    ...
    if (uncategorized > 0) {
      ...
      console.log(`  Estas podem ser transações que não estavam no CSV original ou`);
      console.log(`  que não puderam ser correspondidas devido a diferenças de data/valor.`);
    }
